...
export default class EventManager extends LightningElement {
  ...
  handleCancelSubscribe = () => {
    ...
    const messageCallback = function (response) {
      ...
      if (currentUserId === obj.data.payload.CreatedById) {
        //dispatch the users events instead of all events
        let message = "Order created: " + obj.data.payload.CancellationRecordId;
        let variant = "success";
        if (obj.data.payload.AssetCancelErrorDetailEvents != null) {
          message =
            "Couldn’t cancel the asset: " +
            obj.data.payload.AssetCancelErrorDetailEvents[0].ErrorMessage;
          variant = "error";
        }
    ...
    const messageCallback = function (response) {
      ...
      if (currentUserId === obj.data.payload.CreatedById) {
        ...
        if (obj.data.payload.AssetRenewErrorDetailEvents != null) {
          message =
            "Couldn’t renew the asset: " +
            obj.data.payload.AssetRenewErrorDetailEvents[0].ErrorMessage;
          variant = "error";
        }
        const evt = new ShowToastEvent({
          message: message,
  ...
  handleAmendSubscribe = () => {
    ...
    const messageCallback = function (response) {
      ...
      if (currentUserId === obj.data.payload.CreatedById) {
        ...
        if (obj.data.payload.AssetAmendErrorDetailEvents != null) {
          message =
            "Couldn’t amend the asset: " +
            obj.data.payload.AssetAmendErrorDetailEvents[0].ErrorMessage;
          variant = "error";
        }
        const evt = new ShowToastEvent({
          message: message,
    ...
    subscribe(this.amendChannel, -1, messageCallback).then((response) => {
      // Response contains the subscription information on amend call
