...
trigger UpdateDiscountOnCart on Quote (after update) {
     ...
     if(Trigger.isAfter && Trigger.isUpdate){
         ...
         List<QuoteLineItem> qLineList = Test.isRunningTest() ? [SELECT ID, Subtotal, cartitemid__c, UnitPrice, QuoteId, TotalPrice, Discount FROM QuoteLineItem WHERE QuoteId IN: quoteId] : [SELECT ID, Subtotal, cartitemid__c, UnitPrice, QuoteId, TotalPrice, Discount FROM QuoteLineItem WHERE QuoteId IN: quoteId AND DISCOUNT > 0 WITH SECURITY_ENFORCED];
