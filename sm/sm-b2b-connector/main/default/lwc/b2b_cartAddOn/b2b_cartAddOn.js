...
import searchCartItems from '@salesforce/apex/B2B_CartController.searchCartItems';
import addOnCartProducts from '@salesforce/apex/B2B_RelatedProductsController.getAddOnProductsWithCategoriesByProductList';
...
export default class B2b_cartAddOn extends NavigationMixin(LightningElement) {
    ...
    searchCart(){
        searchCartItems({
            ...
            .then((res) => {
                this.cartItemsproducts = this.getCartItemIds(res);
    ...
    getCartItemIds(result) {
        let test = [];
        result.cartItems.forEach(item => {
            test.push(item.cartItem.productId)
        });
        return test;
      }
    ...
    triggerProductSearch() {
        addOnCartProducts({
            productIds: this.cartItemsproducts,
        })
            .then((result) => {
                const productIds = [];
                const categoryIds = [];
